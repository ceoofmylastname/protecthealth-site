...
- `licenseNumber` is empty on all three and **is staying that way**. Verified Aug 12 2026: Nevada has no requirement to display a producer license number on a website or in advertising, unlike California (Ins. Code 1725.5). The number is public and searchable by name via the Nevada DOI and NIPR lookups, so publishing it adds almost nothing. Do not chase it. Caveat: if a campaign emails or mails into California, 1725.5 does apply there.
- `sameAs`: **Robert's LinkedIn landed Aug 12 2026.** **Brian does not have a LinkedIn account, confirmed Aug 12 2026 — stop asking for it.** His `sameAs` stays empty, his profile renders no link block, nothing false is published and there is no empty placeholder. Brenda's is still unknown. `personSchema()` omits empty values, so never invent one.
- **Robert's and Brian's bios are their own words**, supplied Aug 12 2026, and `AUTHORS.short` was rewritten for both so the Person description does not contradict the visible bio. Do not rewrite either without asking them. Note Robert describes himself as COO rather than as an owner, and Brian's edit removed the "one of" hedge so the copy now asserts flatly that ProtectHealth is "Nevada's largest" agency — a factual superlative in an industry where NRS 686A.030 prohibits misleading advertising. That was his call as CEO; leave it unless he revisits it.
- **Brenda Morgen's bio is STILL a DRAFT pending her approval.** She had no bio at all before Jul 27. What is there now asserts only her role, agency and city — no years of experience, no carrier appointments. She carries the byline on 4 posts and their Q&A children, so she must read and correct it. Her wording wins. Her photo was replaced Aug 12 2026 but the bio was not.
- **Team photos are mixed provenance as of Aug 12 2026.** Brian's is a real studio photograph he supplied. Brenda's is an AI-rendered likeness on a matching grey background. The other four are still Nano Banana re-renders on the older light blue gradient. The two new ones are self-hosted at `/assets/team/{slug}.webp`. A real photograph is the stronger trust signal, so prefer one whenever a person supplies it.
